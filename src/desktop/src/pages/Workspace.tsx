...
export function WorkspacePage() {
  ...
  const joinWorkspace = async () => {
    ...
    setActionLoading(true);
    try {
      const res = await authFetch('/api/v1/workspaces/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const leaveWorkspace = async (workspaceId: string) => {
    ...
    try {
      const res = await authFetch(`/api/v1/workspaces/${workspaceId}/leave`, {
        method: 'POST',
      });
  ...
  const regenerateInviteCode = async (workspaceId: string) => {
    try {
      const res = await authFetch(`/api/v1/workspaces/${workspaceId}/regenerate-invite`, {
        method: 'POST',
      });
      ...
      <div className="cyber-card" style={{ marginBottom: 'var(--gap-xl)' }}>
        ...
        <div className="cyber-card-body">
          <div style={{ display: 'flex', gap: 'var(--gap-xl)' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.5rem', fontFamily: 'var(--font-display)', color: 'var(--success)' }}>
                {totalMembers}
              </div>
              <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>
                Connections
              ...
              <div key={ws.id} className="cyber-card hover-lift">
                <div className="cyber-card-body" style={{ padding: 'var(--gap-lg)' }}>
                  <div style={{
                    ...
                    <div>
                      <div style={{
                        ...
                        {ws.isPrivate ? <Lock size={14} style={{ color: 'var(--text-muted)' }} /> : <Globe size={14} style={{ color: 'var(--success)' }} />}
                        <span style={{
                          fontFamily: 'var(--font-display)',
                          fontSize: '1.1rem',
                          color: 'var(--text-primary)',
                    ...
                    <div style={{
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: 'none',
                          cursor: 'pointer',
                          color: copied === ws.inviteCode ? 'var(--success)' : 'var(--text-muted)',
                          padding: '4px',
                        }}
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Zap size={14} style={{ color: ws.nodeCount > 0 ? 'var(--success)' : 'var(--text-muted)' }} />
                      <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
